...
(* Changing OutputTableau to True gives the final Tablau instead of the value. Note that there may remain artificial variables and slack variables in the basis, so this is not immediately useful to give the vector achieving the optimum in terms of the original problem *)
Options[LPSolveAssumptions]={Iterations->50000,  PrintTemp->False, OutputValues->False,OutLength->-1};
LPSolveAssumptions[A_,assump_,st_,current_,OptionsPattern[]]:=Module[{col,row,j,it,Tab,dimr,dimc,ones,extra={},cur=current,nf=0},Tab=A;{dimr,dimc}=Dimensions[Tab];it=1;While[it<OptionValue[Iterations],it++;For[col=1,col<=dimc-1,col++,If[Simplify[Tab[[1,col]]>0,assump],Break[]]];If[col==dimc,Break[]];For[row=st,row<=dimr,row++,If[Simplify[Tab[[row,col]]>0,assump],Break[]]];If[row==dimr+1,nf=1;Print["LPSolveAssumptions: Not found positive entry in column ",col,": ",Transpose[Tab][[col]]];Break[]];For[j=row+1,j<=dimr,j++,If[Simplify[Tab[[row,col]]==0,assump],row=j,If[Simplify[Tab[[j,col]]>0,assump]&&Simplify[(Tab[[j,dimc]]/Tab[[j,col]]-Tab[[row,dimc]]/Tab[[row,col]])<0,assump],row=j]]];Tab=ReplacePart[Tab,row->Simplify[Tab[[row]]/Tab[[row,col]]]];For[j=1,j<=dimr,j++,If[j!=row,Tab=ReplacePart[Tab,j->Simplify[Tab[[j]]-Tab[[row]]*Tab[[j,col]]/Tab[[row,col]]]]]]];If[it==OptionValue[Iterations],Print["LPSolveAssumptions: Optimum not found after ",OptionValue[Iterations] ," iterations."]];If[nf==1,cur=Insert[cur,{assump,\[Infinity]},-1],ones=DeleteDuplicates[Simplify[Tab[[1]]]];
...
Options[PhaseIIAssumptions]={Iterations-> 50000, PrintTemp->False,OutputValues->False,OutLength->-1};
PhaseIIAssumptions[A_,assump_,OptionsPattern[]]:=LPSolveAssumptions[Transpose[Drop[Transpose[Drop[A,1]],{Dimensions[A][[2]]-Dimensions[A][[1]]+2,Dimensions[A][[2]]-1}]],assump,2,{},{Iterations-> OptionValue[Iterations], PrintTemp-> OptionValue[PrintTemp],OutputValues->OptionValue[OutputValues],OutLength->OptionValue[OutLength]}]

Options[LPAssumptions]={Iterations-> 50000, PrintTemp->False,OutputValues->False};
LPAssumptions[c_,M_,b_,assump:Except[_?OptionQ]:{},OptionsPattern[]]:=Module[{cp,Mp,bp,i,phase1out},{cp,Mp,bp}=SlackenMix[c,M,b];If[cp===Null,Return[]];phase1out=PhaseI[StartTab[cp,Mp,bp],assump,Iterations->OptionValue[Iterations]];If[OptionValue[PrintTemp],PrintTemporary["Phase I finished."]];PhaseIIAssumptions[phase1out,assump,{Iterations->OptionValue[Iterations], PrintTemp->OptionValue[PrintTemp],OutputValues->OptionValue[OutputValues],OutLength->Dimensions[c][[1]]}]]
